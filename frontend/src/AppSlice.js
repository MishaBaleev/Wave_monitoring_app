...
export const appSlice = createSlice({
    ...
    reducers: {
        ...
        addMissionElement: (state, action) => {
            state.mission.push({id: action.payload.id, coords: [action.payload.coords[0], action.payload.coords[1], 3]})
        ...
        updateMissionElement: (state, action) => {
            ...
            if (command_type === "coords"){
                state.mission = state.mission.map(item => 
                    item.id === action.payload.id ? {...item, coords: [action.payload.value[0], action.payload.value[1], item.coords[2]]} : item
                )
            }
            if (command_type === "alt"){
                state.mission = state.mission.map(item => 
                    item.id === action.payload.id ? {...item, coords: [item.coords[0], item.coords[1], action.payload.value]} : item
                )
            }
